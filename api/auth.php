...
try {
    switch ($action) {
        ...
        case 'verifica_otp':
            ...
            $token = JWT::generate(['id'=>$dati['id'],'sub'=>$dati['id'],'nome'=>$dati['nome'],'cognome'=>$dati['cognome'],'email'=>$dati['email'],'ruolo'=>$dati['ruolo']]);
        ...
        /* --------------------------------------------------
           ...
           POST body: password_attuale, nuova_password
           -------------------------------------------------- */
        case 'change_password':
            $payload = requireAuth();
            $id = $payload['id'] ?? $payload['sub'] ?? 0;
            if (empty($body['password_attuale']) || empty($body['nuova_password']))
                throw new ApiException('Campi obbligatori mancanti', 400);
            if (strlen($body['nuova_password']) < 8)
                throw new ApiException('Password troppo corta (min. 8 caratteri)', 400);

            $utente = DB::fetchOne('SELECT id, password_hash FROM utenti WHERE id = ?', [$id]);
            if (!$utente || !password_verify($body['password_attuale'], $utente['password_hash']))
                throw new ApiException('Password attuale non corretta', 401);

            $hash = password_hash($body['nuova_password'], PASSWORD_BCRYPT, ['cost' => 12]);
            DB::query('UPDATE utenti SET password_hash = ? WHERE id = ?', [$hash, $id]);
            jsonResponse(['success' => true, 'message' => 'Password aggiornata']);
            break;
