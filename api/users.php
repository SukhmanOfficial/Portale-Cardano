...
try {
    switch ($action) {
        ...
        case 'figli':
            $me   = requireAuth(['genitore','segreteria','admin']);
            $data = DB::fetchAll(
                "SELECT f.id, f.nome, f.cognome, f.scuola, f.citta_scuola, s.nome AS nome_scuola_media
                 ...
                 WHERE f.id_genitore = ?",
                [($me['id'] ?? ($me['id'] ?? $me['sub'] ?? 0) ?? 0)]
            );
            echo json_encode(['success' => true, 'data' => $data]);
            break;

        case 'add_figlio':
            $me = requireAuth(['genitore']);
            $body = json_decode(file_get_contents('php://input'), true) ?? [];
            $nome    = trim($body['nome']    ?? '');
            $cognome = trim($body['cognome'] ?? '');
            $scuola  = trim($body['scuola']  ?? '');
            if (!$nome || !$cognome) throw new ApiException('Nome e cognome obbligatori');
            $id = DB::insert('figli', ['id_genitore'=>($me['id'] ?? ($me['id'] ?? $me['sub'] ?? 0) ?? 0),'nome'=>$nome,'cognome'=>$cognome,'scuola'=>$scuola]);
        ...
        case 'del_figlio':
            $me = requireAuth(['genitore']);
            $id = (int)($_GET['id'] ?? 0);
            DB::delete('figli', 'id = ? AND id_genitore = ?', [$id, ($me['id'] ?? ($me['id'] ?? $me['sub'] ?? 0) ?? 0)]);
            echo json_encode(['success' => true]);
            break;
