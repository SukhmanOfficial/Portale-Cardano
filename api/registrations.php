    ...
    switch ($action) {
        ...
        case 'create':
            ...
            $figlio = DB::fetchOne('SELECT id FROM figli WHERE id = ? AND id_genitore = ?',
                [$id_figlio, ($payload['id'] ?? $payload['sub'] ?? 0)]);
            ...
            $id_iscrizione = DB::insert('iscrizioni', [
                'id_figlio'    => $id_figlio,
                'id_evento'    => $id_evento,
                'id_genitore'  => ($payload['id'] ?? $payload['sub'] ?? 0),
            ...
            // Invia email con QR
            $genitore = DB::fetchOne('SELECT email, nome FROM utenti WHERE id = ?', [($payload['id'] ?? $payload['sub'] ?? 0)]);
        ...
        case 'cancel':
            ...
            if ($payload['ruolo'] === 'genitore' && $iscr['id_genitore'] !== ($payload['id'] ?? $payload['sub'] ?? 0))
