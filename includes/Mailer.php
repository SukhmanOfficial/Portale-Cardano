...
class Mailer
{
    ...
    {
        // === DEVELOPMENT: logga l'email invece di inviarla ===
        if (APP_ENV === 'development') {
            $logLine = implode(" | ", [
                "[MAIL-DEV]",
                "TO: $to",
                "SUBJECT: $subject",
                "BODY: " . substr(strip_tags($html), 0, 200),
            ]);
            error_log($logLine);
            return;
        }

        // === PRODUZIONE: PHPMailer con SMTP autenticato ===
        if (MAIL_USER !== '' && MAIL_PORT !== 25 && file_exists(__DIR__ . '/../vendor/autoload.php')) {
        ...
        // === POSTFIX locale (porta 25, no autenticazione, no TLS) ===
        if (MAIL_PORT == 25) {
            require_once __DIR__ . '/../vendor/autoload.php';
            $mail = new PHPMailer\PHPMailer\PHPMailer(true);
            $mail->isSMTP();
        ...
        if (!$sent) {
            error_log("[Mailer] FALLBACK mail() fallita — TO: $to | SUBJECT: $subject");
        }
    }
